        ...
        return this.state.teams.map((team, i) => {
            return (
                <div key={i} className='teamListTab'>
                    <div className='removeTeamButton' onClick={() => this.removeTeam(i)}>
                        <DeleteForeverIcon style={{ fontSize: '40px', fill: '#FFC85D' }} />
                    </div>
                    <div className='flex-row teamInfoTop'>
                        <span style={{ marginRight: '50px', color: 'rgb(255, 183, 81)' }}>{team.name}</span>
                        <span className='showOnPc'>Avaliable PVE chances: <span style={{ color: 'rgb(255, 183, 81)' }}>NA</span></span>
                    </div>
                        ...
                        if (id !== '') {
                            ...
                            return (
                                <div key={id} className='flex-column'>
                                    <div className='flex-row showOnMobile' style={{ justifyContent: 'space-around', width: '100%', minWidth: '100px', color: 'white', fontWeight: '500', justifyContent: 'flex-start', marginTop: '12px' }}>
                                        <span style={{ width: '33%' }}>{char.name}</span>
                                        <span style={{ width: '33%', color: 'rgb(255, 183, 81)' }}>Level: {baby.level}</span>
                                        <span style={{ width: '33%' }}>{baby.location}</span>
                                    </div>
                                    <div className='flex-row' style={{ alignItems: 'center', height: '100px', marginBottom: '12px', marginTop: '12px', color: 'white', fontSize: '14px', fontWeight: '500' }}>
                                        <div>
                                            <img alt={char.name} style={{ height: '100px', width: '100px' }} src={char.listImgUrl} />
                                        </div>
                                        <div className='flex-column showOnPc' style={{ height: '100%', justifyContent: 'space-around', marginLeft: '10px', borderRight: '1px solid rgb(255, 183, 81)', width: '12%', minWidth: '100px' }}>
                                            <span >{char.name}</span>
                                            <span style={{ color: 'rgb(255, 183, 81)' }}>Level: {baby.level}</span>
                                            <span >{baby.location}</span>
                                        </div>
                                        <div className='flex-column teamedBabyInfoWrapper'>
                                            <div className='flex-row' style={{ width: '100%' }}>
                                                <div className='flex-column centered' style={{ width: '25%' }}>
                                                    <span>Total</span>
                                                    <span style={{ marginTop: '1px', color: 'rgb(255, 183, 81)' }}>{baby.attr.reduce((a, b) => a + b, 0)}</span>
                                                </div>
                                                {baby.attr.slice(0, 3).map((a, i) => {
                                                    return (
                                                        <div key={i} className='flex-column centered' style={{ width: '25%' }}>
                                                            ...
                                                            <span style={{ marginTop: '1px', color: 'rgb(255, 183, 81)' }}>{a}</span>
                                                        </div>
                                                    )
                                                })}
                                            </div>
                                            <div className='flex-row' style={{ width: '100%', marginTop: '15px' }}>
                                                <div className='flex-column centered' style={{ width: '25%' }}>
                                                    <span>Rarity</span>
                                                    <span style={{ marginTop: '1px', color: 'rgb(255, 183, 81)' }}>{char.rarityName}</span>
                                                </div>
                                                {baby.attr.slice(3).map((a, i) => {
                                                    return (
                                                        <div key={i} className='flex-column centered' style={{ width: '25%' }}>
                                                            <span style={char.attr1 === Constants.attributes[i + 3] || char.attr2 === Constants.attributes[i + 3] ? { color: 'rgb(255, 183, 81)' } : {}}>{Constants.attributes[i + 3]}</span>
                                                            <span style={{ marginTop: '1px', color: 'rgb(255, 183, 81)' }}>{a}</span>
                                                        </div>
                                                    )
                                                })}
                                    ...
                                    <div className='flex-row showOnMobile' style={bi === 2 ? { width: '100%', paddingBottom: '15px', marginTop: '-12px', zIndex: '10' } : { width: '100%', paddingBottom: '15px', marginTop: '-12px', zIndex: '10', borderBottom: '1px solid rgb(52, 61, 79)' }}>
                                        ...
                                        <div className='flex-row centered' style={{ width: 'calc(100% - 100px)', justifyContent: 'space-around' }}>
                                            ...
                                            {
                                                baby.location !== 'Home' ?
                                                    <Button variant="text" style={{ background: 'rgb(254, 153, 59)', color: 'white', borderRadius: '10px', fontWeight: 'bold', width: '100px', height: '32px', fontSize: '12px' }} onClick={() => this.withdrawBaby(baby, id)}>Quit Work</Button> :
                                                    <Button variant="text" style={{ background: 'rgb(254, 153, 59)', color: 'white', borderRadius: '10px', fontWeight: 'bold', width: '100px', height: '32px', fontSize: '12px' }} onClick={() => this.startWorkBaby(baby, id)}>Start Work</Button>
                                            }
                                        </div>
                                    </div>
                                </div>
        ...
        return (
            <div id={'teamListContainer'} className='flex-column centered'>
                <div className='teamListTab' style={{ marginTop: '40px' }}>
                    ...
                    <Button variant="text" style={{ background: 'rgb(254, 153, 59)', color: 'white', paddingLeft: '30px', paddingRight: '30px', borderRadius: '17px', marginTop: '22px', fontWeight: 'bold', marginLeft: '20px' }} onClick={() => this.setState({ importDialog: true, importTeams: '' })}>Import</Button>
                </div>
                {this.renderTeamList()}
                <Dialog
                ...
                <Dialog
                    ...
                    <div className='teamListDialogOutline flex-column centered'>
                        <TextField autoFocus margin="dense" id="name" label="Import teams" type="text" fullWidth variant="standard" value={this.state.importTeams} onChange={(e) => this.setState({ importTeams: e.target.value })} />
                        <Button variant="text" style={{ background: 'rgb(254, 153, 59)', color: 'white', borderRadius: '10px', fontWeight: 'bold', width: '70%', height: '32px', marginTop: '20px' }} onClick={() => { 
                            this.props.updateTeams(JSON.parse(this.state.importTeams));
